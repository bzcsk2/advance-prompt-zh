...
request-only (no business rules), never leaks ``denied_reasons``, and always
returns the validated ``AnswerEnvelope`` (success or abstain).
"""
...
def test_v1_chat_sufficient_returns_envelope() -> None:
    ...
    resp = client.post("/v1/chat", json=_body())
    ...
    assert data["request_id"] == "r-test"  # ctx built from request metadata
...
def test_v1_chat_backend_error_returns_503() -> None:
    client = _client(
        _build_service(RuntimeError("qdrant down"), ClaimExtraction(draft_answer="x", claims=[]))
    )
    resp = client.post("/v1/chat", json=_body())
    assert resp.status_code == 503
    assert "FastPathBackendError" not in resp.text or "retrieval" in resp.text.lower()


def test_v1_chat_missing_required_field_returns_422() -> None:
    body = _body()
    del body["tenant_id"]
    client = _client(
